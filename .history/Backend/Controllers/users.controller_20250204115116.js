...
import jwt from 'jsonwebtoken';  // Assuming you're using the 'jsonwebtoken' package for generating JWT tokens
    ...
    try {
        ...
        bcrypt.compare(password, result.rows[0].password, (err, same) => {
            ...
            const token = jwt.sign(
                ...
                process.env.JWT_SECRET, // Your secret key, should be stored in environment variables
        ...
        console.log("Error in users login controller", error.message);
        res.status(500).json({ message: "Internal Server Error" });
    }
}
